...
class AcquisitionThread(QThread):
    ...
    def run(self):
        ...
        # Se não for série, executar apenas uma vez
        if not self.is_series:
            try:
                self.progress.emit("Adquirindo dados...")
                ...
                print(f"Erro na aquisição: {str(e)}")
                self.error.emit(f"Erro na aquisição: {str(e)}")
                tear_down(self.dig)
                self.finished.emit("error")
                return
        
        # Se for série, executar o loop contínuo
        while True:
